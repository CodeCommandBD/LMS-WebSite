...
const CourseCard = ({ course, isEnrolled: isEnrolledProp }) => {
  ...
  return (
    <div
      ...
      className="group bg-white dark:bg-gray-800 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full border border-gray-100 dark:border-gray-700/50"
    >
      <Link to={`/courseDetails/${course._id || course.id}`}>
        ...
        <div className="relative p-3">
          <div className="relative aspect-16/10 overflow-hidden rounded-2xl bg-gray-100">
            ...
            <div
              className={`absolute top-3 right-3 backdrop-blur-md text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-white/50 ${isEnrolled ? "bg-green-600/90 text-white" : "bg-white/90 dark:bg-slate-900/90 text-blue-600 dark:text-white"}`}
            >
        ...
        <div className="p-5 pt-1 flex flex-col grow">
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-purple-600 dark:text-white uppercase tracking-widest bg-purple-50 dark:bg-purple-900/30 px-2 py-0.5 rounded">
              {course.category || "General"}
            </span>
            <div className="flex items-center gap-1 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded">
          ...
          <h2 className="text-base font-extrabold text-gray-900 dark:text-white mb-6 line-clamp-2 leading-tight group-hover:text-blue-600 transition-colors">
            {course.courseTitle}
          </h2>
          ...
          <div className="flex items-center justify-between mt-auto">
            <div className="flex items-center gap-2">
              <Avatar className="h-8 w-8 ring-2 ring-gray-50 dark:ring-gray-800">
                <AvatarImage src={course.creator?.profilePicture || course.creator?.photoUrl} />
                <AvatarFallback className="bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300 text-[10px]">
