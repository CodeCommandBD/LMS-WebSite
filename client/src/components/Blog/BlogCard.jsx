...
const BlogCard = ({ blog }) => {
  ...
  return (
    <div className="group cursor-pointer flex flex-col h-full w-full rounded-[2rem] bg-white dark:bg-slate-900 p-3 shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-md hover:border-purple-600/30 transition-all duration-300 relative overflow-hidden">
      <Link
      ...
      >
        <div className="relative h-56 w-full">
          <img
            ...
            alt={title}
          />
          <div className="absolute top-4 left-4 rounded-lg bg-white/90 dark:bg-slate-900/90 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-white backdrop-blur-md shadow-sm">
      ...
      <div className="flex flex-col flex-1 px-3 pb-3">
        ...
        <Link to={`/blog/${slug || blog._id}`}>
          <h3 className="mb-3 text-xl font-bold text-slate-900 dark:text-white group-hover:text-purple-600 transition-colors line-clamp-2 leading-snug font-display">
            {title}
          </h3>
        </Link>
