...
const Navbar = () => {
  const { user } = useSelector((state) => state.auth);
  const { siteName, siteLogo } = useSelector((state) => state.settings);
              ...
              {!user ? (
                ...
                <div className="flex items-center gap-3 md:gap-4 ml-2 md:ml-4">
                  ...
                  >
                    ...
                    {cartItems.length > 0 && (
                      <span className="absolute -top-0.5 -right-0.5 h-4 w-4 flex items-center justify-center rounded-full bg-indigo-500 text-[9px] font-black text-white ring-2 ring-purple-600/90 shadow-lg">
                        {cartItems.length}
                      </span>
                    )}
                  </Link>
